...
public class TorolController {
    ...
    @FXML
    private void handleJelentkezoDelete() {
        int id = Integer.parseInt(jelentkezoComboBox.getValue());
        String name = nameField.getText();
        String gender = genderComboBox.getValue();
        if (id != -1) {
            boolean success = recordDAO.deleteJelentkezo(id);
            if (success) {
                System.out.println("Jelentkező törölve: " + name + ", " + gender);
            }
        }
    }
    ...
    private void handleKepzesDelete() {
        ...
        if (id != -1) {
            ...
            if (success) {
                System.out.println("Képzés törölve: " + courseName + ", " + capacity + ", " + minScore);
            }
        }
    }
    ...
    @FXML
    private void handleJelentkezesDelete() {
        ...
        try {
            ...
            boolean success = recordDAO.deleteJelentkezes(applicantId, courseId);
            if (success) {
                System.out.println("Jelentkezés törölve: Jelentkező ID = " + applicantId +
                        ", Képzés ID = " + courseId +
                        ", Rank = " + rank +
                        ", Score = " + score);
            } else {
                System.out.println("A jelentkezés törlése sikertelen.");
            }
        } catch (NumberFormatException e) {
